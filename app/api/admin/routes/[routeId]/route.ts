...
import { deleteWalkingRoute, updateWalkingRoute, type RouteStop } from "@/lib/routesData";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ routeId: string }> }) {
  ...
  const route = await updateWalkingRoute(routeId, {
    name: name.trim(),
  ...
  if (!route) return NextResponse.json({ error: "找不到這條路線" }, { status: 404 });
  return NextResponse.json({ ok: true, route });
}

export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ routeId: string }> }) {
  const { routeId } = await params;
  const ok = await deleteWalkingRoute(routeId);
  if (!ok) return NextResponse.json({ error: "找不到這條路線" }, { status: 404 });
